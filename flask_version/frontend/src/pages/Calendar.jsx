...
export default function Calendar() {
  ...
  const borderCol  = isDark ? "rgba(255,255,255,0.12)" : "rgba(255,79,139,0.18)";
  ...
  const fetchGcalEvents = useCallback(async () => {
    ...
    try {
      ...
      const events = (data.items || []).map(e => {
        ...
        return { id: e.id, title: e.summary || "Untitled Event", description: e.description || "", due_date, link: e.htmlLink || "", isGcal: true };
      }).filter(e => e.due_date);
  ...
  // ── Fetch Philippine Holidays (Nager.Date — no API key needed!) ────────
  ...
  // Fetch current year + next year, re-fetch when year changes
  useEffect(() => {
    fetchHolidays(year);
  ...
  const monthTasks    = tasks.filter(t => { if (!t.due_date) return false; const d = new Date(t.due_date); return d.getFullYear() === year && d.getMonth() === month; });
  const monthGcal     = gcalEvents.filter(e => e.due_date.getFullYear() === year && e.due_date.getMonth() === month);
  const monthHolidays = holidays.filter(h => { const d = new Date(h.date); return d.getFullYear() === year && d.getMonth() === month; });
  ...
  const goToday = () => { const now = new Date(); setSelectedDate(now); setViewDate(new Date(now.getFullYear(), now.getMonth(), 1)); };
  ...
  const toggleMiniTodo = (id) => setMiniTodos(prev => prev.map(t => t.id === id ? { ...t, done: !t.done } : t));
  const deleteMiniTodo = (id) => setMiniTodos(prev => prev.filter(t => t.id !== id));

  return (
    <Box sx={{ width: "100%", display: "flex", flexDirection: { xs: "column", xl: "row" }, gap: { xs: 2, md: 3 }, alignItems: "stretch", minHeight: "calc(100vh - 88px)" }}>

      {/* ── LEFT: Date info + On This Day ─────────────────────────────── */}
      <Box sx={{ width: { xs: "100%", xl: 320 }, flex: { xs: "1 1 auto", xl: "0 0 320px" }, display: "flex", flexDirection: { xs: "column", md: "row", xl: "column" }, gap: 2 }}>

        {/* Date card */}
        <Paper sx={{ p: 3, borderRadius: 4, bgcolor: bgPaper, boxShadow: "0 8px 24px rgba(0,0,0,0.06)", flex: 1 }}>
          <Box sx={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", mb: 2 }}>
            <Box>
              <Typography variant="h5" fontWeight={700} color={textMain} sx={{ fontFamily: "'Playfair Display', serif" }}>
                {selectedDate.toLocaleDateString("en-US", { month: "long", day: "numeric", year: "numeric" })}
              </Typography>
              <Typography variant="body2" color={textMuted} mt={0.5}>Selected Schedule</Typography>
            </Box>
            <Chip label="Today" size="small" onClick={goToday}
              sx={{ borderRadius: 50, bgcolor: primary, color: "#fff", cursor: "pointer", fontWeight: 700 }} />
          </Box>

          {/* Holiday pill */}
          <Box sx={{ display: "flex", gap: 1, flexWrap: "wrap", mb: 2 }}>
            {selectedHoliday ? (
              <Chip label={`🇵🇭 ${selectedHoliday.name}`} size="small"
                sx={{ borderRadius: 50, bgcolor: HOLIDAY_COLOR + "22", color: HOLIDAY_COLOR, fontWeight: 700 }} />
            ) : (
              <Chip label="No holiday" size="small"
                sx={{ borderRadius: 50, bgcolor: isDark ? "rgba(255,255,255,0.08)" : "rgba(0,0,0,0.06)", color: textMuted }} />
            )}
            ...
            {gcalConnected && (
              <Chip label={`${gcalForDate(selectedDate).length} GCal event(s)`} size="small"
                sx={{ borderRadius: 50, bgcolor: gcalColor + "22", color: gcalColor, fontWeight: 700 }} />
            )}
          </Box>
          ...
          <Box sx={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 1.5 }}>
            {[
              ...
              <Paper key={s.label} variant="outlined" sx={{ p: 1.5, borderRadius: 3, textAlign: "center", borderColor: borderCol, bgcolor: "rgba(255,255,255,0.65)" }}>
                <Box sx={{ color: primary, mb: 0.5 }}>{s.icon}</Box>
                <Typography fontWeight={800} fontSize={20} color={textMain}>{s.value}</Typography>
                <Typography variant="caption" color={textMuted}>{s.label}</Typography>
              </Paper>
            ))}
          </Box>
        </Paper>
        ...
        {/* On This Day */}
        <Paper sx={{ p: 3, borderRadius: 4, bgcolor: bgPaper, flexGrow: 1, boxShadow: "0 8px 24px rgba(0,0,0,0.06)", flex: 1 }}>
          ...
          {!gcalConnected && (
            <Box sx={{ p: 2, borderRadius: 3, bgcolor: gcalColor + "11", border: `1px solid ${gcalColor}33`, mb: 2 }}>
              <Typography fontSize={12} color={gcalColor} fontWeight={600}>Log in with Google to see Google Calendar events here.</Typography>
            </Box>
          )}
          ...
          {loadingTasks ? (
            ...
            <Box sx={{ display: "flex", flexDirection: "column", gap: 1.5, maxHeight: 430, overflowY: "auto", pr: 0.5 }}>
              ...
              {/* Holiday card */}
              {selectedHoliday && (
                <Paper variant="outlined" sx={{ p: 2, borderRadius: 3, borderColor: HOLIDAY_COLOR + "55", bgcolor: HOLIDAY_COLOR + "08" }}>
                  <Box sx={{ display: "flex", alignItems: "center", gap: 1 }}>
                    <Typography fontSize={18}>🇵🇭</Typography>
                    <Box>
                      <Typography fontWeight={700} fontSize={14} color={HOLIDAY_COLOR}>{selectedHoliday.name}</Typography>
                      <Typography variant="caption" color={textMuted}>Philippine Public Holiday</Typography>
                    </Box>
                  </Box>
                </Paper>
              )}
              ...
              {selectedItems.map((item, i) => {
                ...
                return (
                  <Paper key={item.id || i} variant="outlined" sx={{ p: 2, borderRadius: 3, borderColor: isGcal ? gcalColor + "44" : borderCol, bgcolor: "rgba(255,255,255,0.75)" }}>
                    <Box sx={{ display: "flex", alignItems: "center", gap: 1 }}>
                    ...
                    {item.description && (
                      <Typography variant="caption" color={textMuted} display="block" mt={0.7} noWrap>{item.description}</Typography>
                    )}
                    <Box sx={{ display: "flex", gap: 1, mt: 1, flexWrap: "wrap" }}>
                      <Chip label={isGcal ? "Google Calendar" : item.category} size="small"
                        sx={{ borderRadius: 50, height: 22, fontSize: 11, bgcolor: isGcal ? gcalColor + "22" : primary + "22", color: isGcal ? gcalColor : primary, fontWeight: 700 }} />
                      {item.due_date && (
                        <Chip label={formatTime(item.due_date)} size="small"
                          sx={{ borderRadius: 50, height: 22, fontSize: 11, bgcolor: borderCol, color: textMuted }} />
                      )}
                      {isGcal && item.link && (
                        <Chip label="Open" size="small" onClick={() => window.open(item.link, "_blank")}
                          sx={{ borderRadius: 50, height: 22, fontSize: 11, bgcolor: gcalColor + "22", color: gcalColor, cursor: "pointer" }} />
                      )}
                    </Box>
                  </Paper>
                );
      ...
      <Box sx={{ flex: "1 1 0", minWidth: 0, display: "flex", flexDirection: "column", overflowX: "auto" }}>
        <Paper sx={{ p: 3, borderRadius: 4, bgcolor: bgPaper, flexGrow: 1, display: "flex", flexDirection: "column", boxShadow: "0 8px 24px rgba(0,0,0,0.06)" }}>

          {/* Month nav */}
          <Box sx={{ display: "flex", alignItems: "center", justifyContent: "space-between", mb: 3 }}>
            <IconButton onClick={() => setViewDate(new Date(year, month - 1, 1))} sx={{ bgcolor: "rgba(0,0,0,0.05)", borderRadius: 3 }}>
              <ChevronLeftIcon />
            </IconButton>
            <Box sx={{ textAlign: "center" }}>
              ...
              <Box sx={{ display: "flex", justifyContent: "center", gap: 1, mt: 1, flexWrap: "wrap" }}>
                <Chip label={`${monthTasks.length} task(s)`} size="small"
                  sx={{ borderRadius: 50, bgcolor: primary + "22", color: primary, fontWeight: 700 }} />
                {monthHolidays.length > 0 && (
                  <Chip label={`${monthHolidays.length} holiday(s)`} size="small"
                    sx={{ borderRadius: 50, bgcolor: HOLIDAY_COLOR + "22", color: HOLIDAY_COLOR, fontWeight: 700 }} />
                )}
                {gcalConnected && (
                  <Chip label={`${monthGcal.length} event(s)`} size="small"
                    sx={{ borderRadius: 50, bgcolor: gcalColor + "22", color: gcalColor, fontWeight: 700 }} />
                )}
              </Box>
            </Box>
          ...
          <Box sx={{ display: "grid", gridTemplateColumns: "repeat(7, minmax(90px, 1fr))", mb: 1, minWidth: 700 }}>
            {WEEKDAYS.map(d => (
              <Typography key={d} align="center" variant="caption" fontWeight={800} color={textMuted} letterSpacing={1} fontSize={11}>{d}</Typography>
            ))}
          </Box>
              ...
              return (
                <Tooltip key={date.toISOString()} title={tooltipParts.join(" · ") || ""} placement="top">
                  <Box onClick={() => setSelectedDate(date)} sx={{
                    ...
                    p: 1.2, borderRadius: 3, cursor: "pointer", position: "relative",
                    border: `1px solid ${holiday && !isToday ? HOLIDAY_COLOR + "55" : isSelected ? primary : borderCol}`,
                    bgcolor: isToday ? primary : isSelected ? selectedBg : "rgba(255,255,255,0.65)",
                    transition: "all 0.18s ease",
                    "&:hover": { transform: "translateY(-3px)", boxShadow: 3, bgcolor: isToday ? primary : primary + "14" },
                  }}>
                    <Typography fontSize={14} fontWeight={isToday || isSelected ? 800 : 600}
                      color={isToday ? "#fff" : holiday ? HOLIDAY_COLOR : textMain}>
                      {date.getDate()}
                    </Typography>

                    <Box sx={{ position: "absolute", bottom: 10, left: 10, display: "flex", gap: 0.5, alignItems: "center" }}>
                      {holiday && (
                        <Box sx={{ width: 7, height: 7, borderRadius: "50%", bgcolor: isToday ? "#fff" : HOLIDAY_COLOR }} />
                      )}
                      {hasTask && (
                      ...
                      {hasGcal && (
                        <Box sx={{ width: 7, height: 7, borderRadius: "50%", bgcolor: isToday ? "#fff" : gcalColor }} />
                      )}
                    </Box>
                  </Box>
                </Tooltip>
      ...
      {/* ── RIGHT: To-do Today ───────────────────────────────────────── */}
      <Box sx={{ width: { xs: "100%", xl: 300 }, flex: { xs: "1 1 auto", xl: "0 0 300px" } }}>
        <Paper sx={{ p: 3, borderRadius: 4, bgcolor: bgPaper, boxShadow: "0 8px 24px rgba(0,0,0,0.06)", height: "100%" }}>
          <Typography fontWeight={800} color={textMain} sx={{ fontFamily: "'Playfair Display', serif" }}>
            To-do Today
          </Typography>
          <Typography variant="body2" color={textMuted} mt={0.5} mb={2}>Minor checklist for today</Typography>

          <Box sx={{ display: "flex", gap: 1, mb: 2 }}>
            <TextField size="small" fullWidth placeholder="Add mini task..."
              value={todoInput} onChange={e => setTodoInput(e.target.value)}
              onKeyDown={e => { if (e.key === "Enter") addMiniTodo(); }}
              sx={{ "& .MuiOutlinedInput-root": { borderRadius: 3, bgcolor: "rgba(255,255,255,0.7)" } }}
            />
            <Button variant="contained" onClick={addMiniTodo} sx={{ minWidth: 44, borderRadius: 3, fontWeight: 800 }}>+</Button>
          </Box>
          ...
          {miniTodos.length === 0 ? (
            ...
            <Box sx={{ display: "flex", flexDirection: "column", gap: 1.2, maxHeight: 430, overflowY: "auto", pr: 0.5 }}>
              {miniTodos.map(todo => (
                <Paper key={todo.id} variant="outlined" sx={{ p: 1.2, borderRadius: 3, display: "flex", alignItems: "center", gap: 1, bgcolor: "rgba(255,255,255,0.7)", borderColor: todo.done ? "#4CAF50" : borderCol }}>
                  <Checkbox checked={todo.done} onChange={() => toggleMiniTodo(todo.id)} size="small" sx={{ p: 0.2, color: primary }} />
                  <Typography fontWeight={700} fontSize={13} color={todo.done ? textMuted : textMain}
                    sx={{ flex: 1, textDecoration: todo.done ? "line-through" : "none", wordBreak: "break-word" }}>
                    {todo.title}
                  </Typography>
