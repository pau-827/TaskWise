...
export default function Calendar() {
  ...
  const gcalForDate = (date) =>
    gcalEvents.filter((e) => isSameDay(e.due_date, date));
  ...
  const monthGcal = gcalEvents.filter(
    (e) => e.due_date.getFullYear() === year && e.due_date.getMonth() === month
  );
      ...
      >
        <Paper sx={{ p: 3, borderRadius: 4, bgcolor: bgPaper, boxShadow: "0 8px 24px rgba(0,0,0,0.06)", flex: 1 }}>
          ...
          <Box sx={{ display: "flex", gap: 1, flexWrap: "wrap", mb: 3 }}>
            <Chip
              label={`${tasksForDate(selectedDate).length} task(s)`}
              size="small"
            ...
            {gcalConnected && (
              <Chip
                label={`${gcalForDate(selectedDate).length} GCal event(s)`}
                size="small"
          ...
          <Box sx={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 1.5 }}>
            {[
              ...
              { icon: <FlagIcon sx={{ fontSize: 20 }} />, label: "GCal", value: monthGcal.length },
            ].map((s) => (
              <Paper key={s.label} variant="outlined" sx={{ p: 1.5, borderRadius: 3, textAlign: "center", borderColor: borderCol, bgcolor: "rgba(255,255,255,0.65)" }}>
                ...
                <Typography fontWeight={800} fontSize={20} color={textMain}>
                  {s.value}
                </Typography>
              ...
              <Tooltip title="Refresh Google Calendar">
                <IconButton size="small" onClick={fetchGcalEvents} disabled={loadingGcal}>
                  <SyncIcon
                    fontSize="small"
                    sx={{
                      animation: loadingGcal ? "spin 1s linear infinite" : "none",
                ...
                return (
                  <Paper key={item.id || i} variant="outlined" sx={{ p: 2, borderRadius: 3, borderColor: isGcal ? gcalColor + "44" : borderCol, bgcolor: "rgba(255,255,255,0.75)" }}>
                    ...
                    <Box sx={{ display: "flex", gap: 1, mt: 1, flexWrap: "wrap" }}>
                      <Chip
                        label={isGcal ? "Google Calendar" : item.category}
                        size="small"
                        sx={{
                          borderRadius: 50,
                          height: 22,
                          fontSize: 11,
                          bgcolor: isGcal ? gcalColor + "22" : primary + "22",
                          color: isGcal ? gcalColor : primary,
                          fontWeight: 700,
                        }}
                      />
                      ...
                      {item.due_date && (
                        <Chip
                          label={formatTime(item.due_date)}
                          size="small"
        ...
        <Paper sx={{ p: 3, borderRadius: 4, bgcolor: bgPaper, flexGrow: 1, display: "flex", flexDirection: "column", boxShadow: "0 8px 24px rgba(0,0,0,0.06)" }}>
          ...
          <Box sx={{ display: "grid", gridTemplateColumns: "repeat(7, minmax(90px, 1fr))", gap: 1, flexGrow: 1, minWidth: 700 }}>
            {calCells.map((date, i) => {
              ...
              return (
                <Tooltip key={date.toISOString()} title={totalItems > 0 ? `${tasksForDate(date).length} task(s), ${gcalForDate(date).length} event(s)` : ""} placement="top">
                  <Box
                    ...
                    sx={{
                      ...
                      cursor: "pointer",
                      position: "relative",
                      border: `1px solid ${isSelected ? primary : borderCol}`,
                  ...
                  >
                    <Typography fontSize={14} fontWeight={isToday || isSelected ? 800 : 600} color={isToday ? "#fff" : textMain}>
                      {date.getDate()}
                    </Typography>
                    ...
                    <Box sx={{ position: "absolute", bottom: 10, left: 10, display: "flex", gap: 0.5 }}>
                      ...
                      {hasGcal && (
                        <Box sx={{ width: 7, height: 7, borderRadius: "50%", bgcolor: isToday ? "#fff" : gcalColor, mt: 0.5 }} />
                      )}
                    </Box>
                  </Box>
                </Tooltip>
              ...
              {miniTodos.map((todo) => (
                <Paper
                  ...
                  sx={{
                    ...
                    gap: 1,
                    bgcolor: "rgba(255,255,255,0.7)",
                    borderColor: todo.done ? "#4CAF50" : borderCol,
                  }}
                >
                  <Checkbox
                    checked={todo.done}
                    onChange={() => toggleMiniTodo(todo.id)}
                    size="small"
                    sx={{ p: 0.2, color: primary }}
                  />

                  <Typography
                    fontWeight={700}
                    fontSize={13}
                    color={todo.done ? textMuted : textMain}
                    sx={{
                      flex: 1,
